...
class StepperController:
    ...
    def __init__(self, i2c_address, stepper_names):
        ...
        for name in stepper_names:
            ...
            bit_position += 2  # Move to next available for STEP/DIR
            limit_switch_position += 2  # next available for limit switches
    ...
    def move_stepper_adaptive(
        self, stepper_name, steps, fast_rate, slow_rate, direction
    ):
        """
    ...
    def home_stepper(self, stepper_name):
        ...
        while stepper["CLOSED_LIMIT"].value:  # Wait for closed limit switch
            stepper["STEP"].value = True
            time.sleep(0.01)
            stepper["STEP"].value = False
            time.sleep(0.01)
...
"""
      ...
      |              |            | GPB1 -----|--------------| DIR
      |              |            |           |
      |              |            | GPA0 -----|--- Limit Switch (Closed)
      |              |            | GPA1 -----|--- Limit Switch (Open)
      |              |            |           |
"""
